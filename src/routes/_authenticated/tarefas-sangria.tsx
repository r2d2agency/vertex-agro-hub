...
function TarefasSangriaPage() {
  ...
  return <div>
    ...
    {loading ? <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">Carregando...</CardContent></Card> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.map((task) => <Card key={task.id}><CardContent className="flex items-start justify-between p-5"><div><p className="font-semibold">{task.label}</p><p className="font-mono text-xs text-muted-foreground">{task.code}</p><p className="mt-2 text-xs text-muted-foreground">{task.active ? "Ativa" : "Inativa"}</p></div><div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => setEditing(task)}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" className="text-destructive" onClick={() => setToDelete(task)}><Trash2 className="h-4 w-4" /></Button></div></CardContent></Card>)}</div>}
...
function TaskDialog({ open, initial, companyId, onOpenChange, onSaved }: { open: boolean; initial?: TappingTask; companyId: string | null; onOpenChange: (open: boolean) => void; onSaved: () => void }) {
  ...
  const openChange = (next: boolean) => { if (next && initial) { setCode(initial.code); setLabel(initial.label); setPosition(String(initial.position)); setActive(initial.active); } if (next && !initial) { setCode(""); setLabel(""); setPosition("0"); setActive(true); } onOpenChange(next); };

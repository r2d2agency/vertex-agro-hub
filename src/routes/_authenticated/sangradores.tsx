...
import { HardHat, Search, Trash2, MapPin, Phone, UserPlus, ShieldAlert, ShieldCheck, Clock3, CheckCircle2, ArchiveX } from "lucide-react";
...
function SangradoresPage() {
  ...
  const approvePreRegistration = useMutation({
    ...
    onSuccess: (person) => {
      ...
      qc.invalidateQueries({ queryKey: ["people", companyId] });
      qc.invalidateQueries({ queryKey: ["tapper-pre-registrations", companyId] });
      setEditingUserId(person.id);
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const rejectPreRegistration = useMutation({
    mutationFn: async (item: TapperPreRegistration) => {
      if (!companyId) throw new Error("Selecione uma empresa");
      return reviewTapperPreRegistration(item.id, {
        companyId,
        status: "rejected",
              ...
              <CardContent className="p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  <Badge variant="secondary" className="gap-1">
                    ...
                    {pendingPreRegistrations.length} pendente(s)
                  </Badge>
                </div>

                {pendingPreRegistrations.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Nenhum pré-cadastro pendente nesta empresa.</p>
                ) : (
                  <div className="grid gap-3">
                    {pendingPreRegistrations.map((item) => (
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            <Button
                              size="sm"
                              className="gap-1"
                              disabled={approvePreRegistration.isPending}
                              onClick={() => approvePreRegistration.mutate(item)}
                            >
                            ...
                            <Button
                              ...
                              variant="outline"
                              className="gap-1"
                              disabled={rejectPreRegistration.isPending}
                              onClick={() => rejectPreRegistration.mutate(item)}
                            >

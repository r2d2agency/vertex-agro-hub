...
export function TapperPlotsDialog({ open, onOpenChange, companyId, tapperKey, tapperName, farmIds }: {
  ...
  const applyTemplate = useMutation({ mutationFn: (templateId: string) => applyTappingTableTemplate({ companyId, tapperKey, farmId, plotId, templateId }), onSuccess: () => { toast.success("Template aplicado ao talhão"); qc.invalidateQueries({ queryKey: ["plot-table-links", companyId, tapperKey] }); }, onError: (e: Error) => toast.error(e.message) });
  ...
  const update = useMutation({ mutationFn: ({ id, active }: { id: string; active: boolean }) => updatePlotTableLink(id, { companyId, active }), onSuccess: () => qc.invalidateQueries({ queryKey: ["plot-table-links", companyId, tapperKey] }), onError: (e: Error) => toast.error(e.message) });
...
function PlotSection({ plot, tapperKey, companyId, open, onToggle, remove, update }: any) {
  ...
  return <div className="rounded-xl border"><button type="button" className="flex w-full items-center justify-between p-3 text-left" onClick={onToggle}>{open ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}<span className="flex-1 px-2 font-medium">{plot.code ? `${plot.code} · ` : ""}{plot.name}</span><span className="text-xs text-muted-foreground">{plot.treeCount ? `${plot.treeCount.toLocaleString("pt-BR")} árvores` : ""}</span></button>{open && <div className="space-y-2 border-t p-3">{query.isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : (query.data ?? []).length === 0 ? <p className="text-xs text-muted-foreground">Nenhuma tabela vinculada a este talhão.</p> : (query.data ?? []).map((link: any) => <div key={link.id} className="flex items-center gap-2 rounded-lg bg-muted/30 px-2 py-2 text-sm"><span className={`flex-1 ${link.active === false ? "line-through text-muted-foreground" : ""}`}>{link.tappingTable?.name ?? "Tabela"} · {link.treeCount ?? "—"} árvores</span><span className="text-xs text-muted-foreground">ordem {link.position + 1}</span><Button size="sm" variant="ghost" onClick={() => update.mutate({ id: link.id, active: link.active === false })}>{link.active === false ? "Ativar" : "Pausar"}</Button><Button size="icon" variant="ghost" onClick={() => remove.mutate({ id: link.id, companyId })}><Trash2 className="h-4 w-4 text-destructive" /></Button></div>)}</div>}</div>;
}
